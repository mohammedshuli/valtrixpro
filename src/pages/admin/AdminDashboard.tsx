...
import type { CateringBooking, CorporateEvent, MealInquiry, Consultation, CourseRegistration, ContactMessage } from '../../types';
...
export default function AdminDashboard() {
  ...
  const [inquiries, setInquiries] = useState<AllInquiries | null>(null);
  ...
  const tabs = [
    { id: 'inquiries', label: '📧 Inquiries', count: inquiries ? Object.values(inquiries).reduce((sum: number, arr: unknown[]) => sum + arr.length, 0) : 0 },
    ...
    <div className="p-8 bg-[#FFF8E7]">
      ...
      <div className="flex gap-4 mb-8 flex-wrap">
        {tabs.map((tab) => (
          ...
          >
            {tab.label}
            {tab.count > 0 && <span className="ml-2 bg-red-500 text-white rounded-full px-2 py-1 text-xs">{tab.count}</span>}
          </motion.button>
        ))}
      </div>
          ...
          <div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-8">
                {/* Catering */}
                <div>
                  <h3 className="text-xl font-semibold text-[#E6A520] mb-4">Catering Inquiries ({inquiries.catering.length})</h3>
                  {inquiries.catering.length > 0 ? (
                ...
                {/* Corporate Events */}
                <div>
                  <h3 className="text-xl font-semibold text-[#E6A520] mb-4">Corporate Events ({inquiries.corporate.length})</h3>
                  {inquiries.corporate.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b-2 border-[#E6A520]">
                        ...
                        <tbody>
                          {inquiries.corporate.slice(0, 5).map((item: CorporateEvent) => (
                            <tr key={item.id} className="border-b hover:bg-gray-50">
                ...
                {/* Meal Inquiries */}
                <div>
                  <h3 className="text-xl font-semibold text-[#E6A520] mb-4">Meal Prep Inquiries ({inquiries.meals.length})</h3>
                  {inquiries.meals.length > 0 ? (
                    <p className="text-gray-600">{inquiries.meals.length} meal prep inquiries</p>
                  ) : (
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              <div className="bg-gradient-to-br from-[#FFD77A] to-[#E6A520] rounded-lg p-6 text-white">
                <div className="text-3xl font-bold">{inquiries?.catering?.length || 0}</div>
              ...
              <div className="bg-gradient-to-br from-[#E6A520] to-[#C68A1A] rounded-lg p-6 text-white">
                <div className="text-3xl font-bold">{inquiries?.corporate?.length || 0}</div>
              ...
              <div className="bg-gradient-to-br from-[#7A4A00] to-[#5A3A00] rounded-lg p-6 text-white">
                <div className="text-3xl font-bold">{inquiries?.meals?.length || 0}</div>
              ...
              <div className="bg-gradient-to-br from-[#8B5A2B] to-[#6B4A1B] rounded-lg p-6 text-white">
                <div className="text-3xl font-bold">{inquiries?.contact?.length || 0}</div>
